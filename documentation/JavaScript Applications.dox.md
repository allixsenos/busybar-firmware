...
Required. The name must equal the manifest `id`. It is 1 to 32 characters long and contains only ASCII alphanumeric characters plus `_`, `-` and `.`. It cannot start with a period. A reverse domain name scheme is recommended, but is not required.
